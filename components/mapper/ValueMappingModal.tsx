...
const ValueMappingModal: React.FC<ValueMappingModalProps> = ({
  ...
  const sourceValues = uniqueValues[activeMapping.sourceLayer]?.[activeMapping.fieldMappings[openValueMapId]] || [];
         ...
         <div className="flex-1 overflow-y-auto p-8 custom-scrollbar space-y-6">
            {sourceValues.length === 0 ? (
              <div className="py-12 flex flex-col items-center justify-center gap-3 text-center">
                ...
                <p className="text-xs text-slate-400 font-medium max-w-xs leading-relaxed">{t.mapper.noValuesDetected}</p>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-2 gap-8 px-4 text-[10px] font-black uppercase text-slate-400 tracking-widest">
                  <span>{t.mapper.sourceUniqueValue}</span>
                  <span>{t.mapper.mapToTarget}</span>
                </div>
                <div className="space-y-3">
                  {sourceValues.map(srcVal => {
