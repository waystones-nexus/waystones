  ...
  return (
    <div className="flex gap-4 group/row">
      <div className="w-8 flex flex-col items-center pt-1">
        ...
        <div className="w-px flex-1 bg-slate-200 my-1 group-last/row:hidden" />
      </div>
      <div className="flex-1 pb-4">
        <div className="flex items-center gap-2 mb-2">
          <span className={`text-xs font-bold ${change.type === 'deleted' ? 'text-rose-500 line-through' : 'text-slate-800'}`}>
            {change.itemName}
          </span>
          <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest px-1.5 py-0.5 rounded bg-slate-100">
          ...
          <div className="space-y-2 mt-3">
            {change.modifiedFields.map((field, fIdx) => (
              <div key={fIdx} className="flex items-center gap-2 flex-wrap">
                <span className="bg-slate-100 text-slate-600 text-[9px] font-black uppercase px-2 py-1 rounded-md tracking-wider border border-slate-200">
                  {field.field}
                </span>
